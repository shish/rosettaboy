...
for lang_runner in glob("*/run*.sh"):
    ...
    if match := re.match("run_(.*).sh", runner):
        sub = match.group(1)
    #print(f"{lang} / {runner} / {sub}")
    proc = subprocess.run(
        [f"./{runner}", "--profile", "600", "--silent", "--headless", "--turbo", "../test_roms/games/opus5.gb"],
        cwd=lang, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
    )
    frames = ""
