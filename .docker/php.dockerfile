...
ARG VERSION
RUN apt update && apt install -y build-essential wget git libxml2-dev libsqlite3-dev libssl-dev autoconf pkg-config libsdl2-dev
RUN adduser --disabled-password dev
USER dev
RUN wget -nv https://www.php.net/distributions/php-${VERSION}.tar.gz -O - \
    | tar -xz -C /tmp
RUN cd /tmp/php-${VERSION} && ./configure --prefix=/home/dev/.php --with-openssl && make install
