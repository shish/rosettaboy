...
pub const CPU = packed struct {
    ...
    fn dump_regs(self: *CPU) {
        let mut op = ram.get(self.pc);
        let op_str = if op == 0xCB {
            op = ram.get(self.pc + 1);
            OP_CB_NAMES[op].to_string()
        } else {
            let base = OP_NAMES[op].to_string();
            let arg = self.load_op(ram, self.pc + 1, OP_TYPES[op]);
            match OP_TYPES[op] {
                0 => base,
    ...
    fn tick_dma(self: *CPU, ram: &mut ram::RAM) {
    ...
    fn tick_clock(self: *CPU, ram: &mut ram::RAM) {
        self.cycle += 1;
        ...
        if ram.get(Mem::TAC) & 1 << 2 == 1 << 2 {
            ...
            let speed = speeds[(ram.get(Mem::TAC) & 0x03)];
    ...
    fn tick_interrupts(self: *CPU, ram: &mut ram::RAM) {
    ...
    fn tick_instructions(self: *CPU, ram: &mut ram::RAM) -> Result<()> {
    ...
    fn load_op(self: *CPU, ram: &ram::RAM, addr: u16, arg_type: u8) -> OpArg {
        match arg_type {
            0 => OpArg {
                u8: 0,
                u16: 0,
    ...
    fn tick_main(self: *CPU, ram: &mut ram::RAM, op: u8) -> Result<()> {
        let arg = {
            let arg_type = OP_TYPES[op];
            let arg_len = OP_LENS[arg_type];
    ...
    fn tick_cb(self: *CPU, ram: &mut ram::RAM, op: u8) {
    ...
    fn set_reg(self: *CPU, n: u8, val: u8) {
            switch(n & 0x07) {
