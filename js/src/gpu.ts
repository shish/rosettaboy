...
export class GPU {
    ...
    headless: boolean;
    debug: boolean;
    hw_window: sdl.SDL_Window | null;
    hw_buffer: sdl.SDL_Texture | null;
    hw_renderer: sdl.SDL_Renderer | null;
    buffer: sdl.SDL_Surface;
    renderer: sdl.SDL_Renderer;
    colors: Array<sdl.SDL_Color>;
    bgp: Array<sdl.SDL_Color>;
    obp0: Array<sdl.SDL_Color>;
    obp1: Array<sdl.SDL_Color>;
    cycle: number;
    ...
    constructor(cpu: CPU, title: string, headless: boolean, debug: boolean) {
        ...
        if (!headless) {
            sdl.f.SDL_Init(sdl.f.SDL_INIT_VIDEO);
            console.log(sdl.f.SDL_GetError());
            this.hw_window = sdl.f.SDL_CreateWindow(
                `RosettaBoy - ${title}`,
                sdl.f.SDL_WINDOWPOS_UNDEFINED,
                0x1FFF0000,//sdl.SDL_WINDOWPOS_UNDEFINED,
                w * SCALE,
                h * SCALE,
                sdl.SDL_WindowFlags.SDL_WINDOW_ALLOW_HIGHDPI |
                sdl.SDL_WindowFlags.SDL_WINDOW_RESIZABLE
            );
            this.hw_renderer = sdl.f.SDL_CreateRenderer(this.hw_window, -1, 0);
            sdl.f.SDL_SetHint(sdl.f.SDL_HINT_RENDER_SCALE_QUALITY, "nearest"); // vs "linear"
            sdl.f.SDL_RenderSetLogicalSize(this.hw_renderer, w, h);
            this.hw_buffer =
                sdl.f.SDL_CreateTexture(this.hw_renderer, sdl.f.SDL_PIXELFORMAT_ABGR8888, sdl.f.SDL_TEXTUREACCESS_STREAMING, w, h);
        } else {
            this.hw_window = null;
        ...
        this.buffer = sdl.f.SDL_CreateRGBSurface(0, w, h, 32, rmask, gmask, bmask, amask);
        this.renderer = sdl.f.SDL_CreateSoftwareRenderer(this.buffer);
    ...
    draw_line(ly: number) {
        ...
        if (lcdc & LCDC.BG_WIN_ENABLED) {
            ...
            if (this.debug) {
                const xy: sdl.SDL_Point = { x: 256 - scroll_x, y: ly };
                //SDL_SetRenderDrawColor(this.renderer, 255, 0, 0, 0xFF);
...
function gen_hue(n: u8): sdl.SDL_Color {
